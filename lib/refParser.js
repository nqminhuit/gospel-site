...
// Keyed by db instance so a reopened db (see lib/bibleDb.js) doesn't reuse
// statements prepared against a now-closed handle.
const preparedCache = new WeakMap();

function getPrepared(db, sql) {
  let byDb = preparedCache.get(db);
  if (!byDb) {
    byDb = new Map();
    preparedCache.set(db, byDb);
  }
...
export function getVersesForRef(db, ref) {
  ...
  for (const seg of segments) {
    ...
    rows.push(...getPrepared(db, sql).all(...params));
  }
  return rows;
}
