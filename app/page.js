  ...
  return (
    <div className="space-y-8">
      <section className="max-w-4xl mx-auto text-center py-6 bg-gradient-to-r from-green-50 to-green-200 border border-green-300 rounded-lg shadow-lg animate-[fadeIn_1s_ease-out_forwards] min-h-[14em]">
        ...
        {isTodaySelected
          ? (<>
            <blockquote className="prose prose-gray max-w-none whitespace-pre-line text-gray-800 leading-8 text-justify mx-4">
              {gospelToday.verses}
            </blockquote>
            <cite className="text-sm text-gray-600 mt-2 block">({gospelToday.ref})</cite>
            {todaySundayLabel && (
              <p className="text-sm text-green-700 font-semibold mt-2">{todaySundayLabel}</p>
            )}
          </>)
          : gospelOfTheDay
            ? (<>
