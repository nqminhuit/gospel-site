...
export default function HomePage() {
  ...
  return (
    <div className="space-y-6 sm:space-y-10">
      <section
        key={selectedDateStr}
        className="relative max-w-3xl mx-auto overflow-hidden rounded-2xl border border-emerald-200/70 bg-white shadow-lg shadow-emerald-900/5 animate-[fadeIn_0.5s_ease-out_forwards] min-h-[14em] dark:border-emerald-900/60 dark:bg-neutral-900"
      >
        ...
        <div className="px-5 py-7 sm:px-10 sm:py-10 text-center">
          <h2 className="text-xl sm:text-2xl font-bold text-emerald-900 mb-5 flex items-center justify-center gap-2 dark:text-emerald-200">
          ...
          {isTodaySelected
            ? (<>
              <blockquote className="font-serif max-w-2xl mx-auto whitespace-pre-line text-neutral-800 leading-8 text-justify text-[0.95rem] sm:text-base dark:text-neutral-200">
                {gospelToday.verses}
              </blockquote>
              <cite className="not-italic text-sm text-neutral-500 mt-3 block dark:text-neutral-400">({gospelToday.ref})</cite>
              {todaySundayLabel && (
                <p className="inline-block text-sm text-emerald-800 font-semibold mt-3 px-3 py-1 rounded-full bg-emerald-50 dark:text-emerald-300 dark:bg-emerald-900/40">{todaySundayLabel}</p>
              )}
            </>)
            : gospelOfTheDay
              ? (<>
                <blockquote className="font-serif max-w-xl mx-auto text-lg text-neutral-800 italic font-medium leading-relaxed dark:text-neutral-200">
                  <q
                    className="cursor-pointer underline decoration-amber-400 decoration-2 underline-offset-4 transition hover:text-emerald-800 active:scale-[0.99] dark:hover:text-emerald-300"
                ...
                <button
                  type="button"
                  onClick={openGospelModal}
                  className="mt-4 text-sm font-semibold text-amber-700 hover:text-amber-800 cursor-pointer dark:text-amber-500 dark:hover:text-amber-400"
                >
                  Đọc trọn bài Tin Mừng →
                </button>
                <cite className="not-italic text-sm text-neutral-500 mt-3 block dark:text-neutral-400">({gospelOfTheDay.reference})</cite>
                <p className="inline-block text-sm text-emerald-800 font-semibold mt-3 px-3 py-1 rounded-full bg-emerald-50 dark:text-emerald-300 dark:bg-emerald-900/40">{gospelOfTheDay.sunday}</p>
              </>)
              : (<>
                <blockquote className="font-serif max-w-xl mx-auto text-lg text-neutral-800 italic font-medium leading-relaxed dark:text-neutral-200">
                  <q>Đây là điều răn của Thầy: anh em hãy yêu thương nhau như Thầy đã yêu thương anh em.</q>
                </blockquote>
                <cite className="not-italic text-sm text-neutral-500 mt-3 block dark:text-neutral-400">(Ga 15,12)</cite>
              </>)}
      ...
      <div className="max-w-xs sm:max-w-sm mx-auto space-y-3">
        ...
        <div className="flex items-center justify-center gap-3 text-sm text-neutral-600 dark:text-neutral-400">
          <span>Ngày được chọn: <strong className="text-neutral-800 dark:text-neutral-200">{date.toLocaleDateString('vi-VN')}</strong></span>
          {!isTodaySelected && (
            <button
              type="button"
              onClick={() => setDate(new Date())}
              className="text-emerald-700 font-semibold hover:text-emerald-900 hover:underline cursor-pointer dark:text-emerald-400 dark:hover:text-emerald-300"
            >
              Hôm nay
            </button>
          )}
